...
void tk_hookMethod_2(Class originalClass, SEL originalSelector, Class swizzledClass, SEL swizzledSelector, Class oldOriginalClass, SEL oldOriginalSelector) {
    Method originalMethod = class_getInstanceMethod(originalClass, originalSelector);
    Method swizzledMethod = class_getInstanceMethod(swizzledClass, swizzledSelector);
    Method oldOriginalMethod = class_getInstanceMethod(oldOriginalClass, oldOriginalSelector);
    if(originalMethod && swizzledMethod && oldOriginalMethod) {
        method_exchangeImplementations(originalMethod, swizzledMethod);
        method_exchangeImplementations(swizzledMethod, oldOriginalMethod);
    }
}
